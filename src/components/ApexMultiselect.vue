...
<template>
  <ApexField v-bind="fieldProps" :value="modelValue" :filled="chips.length > 0 || (isFloat && !!placeholder)" :focused="focused || open"
    ...
    <div ref="root" style="position:relative">
      <div ref="trigger" class="apex-ctl apex-ctl--trigger apex-ctl--multi" role="combobox" :id="id"
        ...
        <span v-for="c in shownChips" :key="String(c.value)" class="apex-chip">
          <img v-if="c.image" class="apex-chip__img" :src="c.image" alt="" />
        ...
        <span v-if="!chips.length" class="apex-ctl__ph">{{ placeholder || t('apexui.select') }}</span>
        <ApexIcon v-if="loading" name="progress_activity" spin class="apex-ctl__icon" />
        <button v-if="clearable && chips.length && !disabled" type="button" class="apex-ctl__btn"
      ...
      <div v-if="open" class="apex-pop" :id="listId" role="listbox" aria-multiselectable="true">
        ...
        <button v-for="(o, i) in opts" :key="String(o.value)" :id="listId + '-' + i" type="button"
                class="apex-pop__opt" role="option" :aria-selected="selected.includes(o.value)"
